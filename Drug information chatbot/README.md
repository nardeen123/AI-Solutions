...
***Document Ingestion*** → Upload & preprocess medical/drug documents.

***Chunking & Embedding*** → Sentence Transformers split text and create embeddings.

***Vector Store*** → Stores embeddings for semantic similarity search.

***Retriever*** → Contextual Compression Retriever fetches top-k relevant passages.

***LLM Response*** → Retrieved context + query passed to LLM with optimized prompts.

***Memory*** → Keeps track of conversation history.

***Interface*** → Text-based chatbot interface.
...
Large Language Models (LLMs) – OpenAI / Hugging Face models
